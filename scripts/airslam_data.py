...
def parse_args(argv: list[str] | None = None):
    ...
    p.add_argument(
        "--index-path",
        default="index/airslam-data.json",
        type=Path,
...
def extract_frames(svo_file: Path, rel_path: Path, output_dir: Path, resolution: str = "640,480"):
    """Extract left and right images from *svo_file* into stereo subfolders under *output_dir / rel_path without suffix*"""
    ensure_cv2()
    ...
    # Create stereo output subfolders
    left_out_dir = frame_root / "cam0" / "data"
    right_out_dir = frame_root / "cam1" / "data"
    ...
    try:
        while True:
            ...
            if frame_id % 10 == 0:
                ...
                zed.retrieve_image(left_mat, sl.VIEW.LEFT, resolution=custom_res)   # type: ignore[attr-defined]
                zed.retrieve_image(right_mat, sl.VIEW.RIGHT, resolution=custom_res) # type: ignore[attr-defined]

                # Prepare output file paths
                left_file = left_out_dir / f"{saved_frame_id:06d}.png"
                right_file = right_out_dir / f"{saved_frame_id:06d}.png"

                # Save using ZED SDK write method
                left_status = left_mat.write(str(left_file))
                right_status = right_mat.write(str(right_file))

                if left_status != sl.ERROR_CODE.SUCCESS or right_status != sl.ERROR_CODE.SUCCESS:  # type: ignore[attr-defined]
